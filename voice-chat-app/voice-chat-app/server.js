...
wss.on('connection', (ws) => {
  ws.on('message', (message) => {
    ...
    if (data.type === 'join') {
      ...
      clients.push(ws);
      ws.roomId = roomId;
      ws.username = data.username || 'Usuário';
      ...
      if (clients.length === 2) {
        ...
        ws.send(JSON.stringify({ type: 'start', playerId: 0 }));
      }
      ...
      // Retransmitir mensagem de texto para outros usuários na sala
      const clients = rooms.get(ws.roomId);
      if (clients) {
        clients.forEach((client) => {
          if (client !== ws && client.readyState === WebSocket.OPEN) {
            client.send(JSON.stringify({
              type: 'message',
              text: data.text,
              username: data.username || ws.username,
